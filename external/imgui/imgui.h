...
// Configuration file with compile-time options (edit imconfig.h or '#define IMGUI_USER_CONFIG "myfilename.h" from your build systems')
...
namespace ImGui
{
    ...
    IMGUI_API void          ShowAboutWindow(bool* p_open = NULL);       // create About window. display Dear ImGui version, credits and build/systems information.
    ...
    IMGUI_API void          SetCursorPos(const ImVec2& local_pos);                          //    are using the main, absolute coordinate systems.
    ...
    IMGUI_API void          SetTabItemClosed(const char* tab_or_docked_window_label);           // notify TabBar or Docking systems of a closed tab/window ahead (useful to reduce visual flicker on reorderable tab bars). For tab-bar: call after BeginTabBar() and before Tab submissions. Otherwise call with a window name.
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_SpanAvailWidth       = 1 << 11,  // Extend hit box to the right-most edge, even if not framed. This is not the default in order to allow adding other items on the same line. In the future we may refactor the hit systems to be front-to-back, allowing natural overlaps and then this can become the default.
...
struct ImGuiStyle
{
    ...
    ImVec2      TouchExtraPadding;          // Expand reactive bounding box for touch-based systems where touch position is not accurate enough. Unfortunately we don't sort widgets so priority on overlap will always be given to the first widget. So don't grow this too much!
...
//  - Upload the pixels data into a texture within your graphics systems (see imgui_impl_xxxx.cpp examples)
