...
struct ImGuiContext
{
    ...
    ImGuiNavLayer           NavLayer;                           // Layer we are navigating on. For now the systems is hard-coded for 0=main contents and 1=menu/title bar, may expose layers later.
    ...
    // Legacy Focus/Tabbing systems (older than Nav, active even if Nav is disabled, misnamed. FIXME-NAV: This needs a redesign!)
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    int                     FocusCounterRegular;    // (Legacy Focus/Tabbing systems) Sequential counter, start at -1 and increase as assigned via FocusableItemRegister() (FIXME-NAV: Needs redesign)
    int                     FocusCounterTabStop;    // (Legacy Focus/Tabbing systems) Same, but only count widgets which you can Tab through.
...
enum ImGuiTabBarFlagsPrivate_
{
    ...
    ImGuiTabBarFlags_SaveSettings               = 1 << 22   // FIXME: Settings are handled by the docking systems, this only request the tab bar to mark settings dirty when reordering tabs
};
