...
class dense_set {
    ...
    [[nodiscard]] auto insert_or_do_nothing(Other &&value) {
        ...
        sparse.first()[index] = packed.first().m_size() - 1u;
        rehash_if_required();
...
public:
    ...
    std::pair<iterator, bool> emplace(Args &&...args) {
        if constexpr(((sizeof...(Args) == 1u) && ... && std::is_same_v<std::decay_t<Args>, value_type>)) {
            ...
            auto &node = packed.first().emplace_back(std::piecewise_construct, std::make_tuple(packed.first().m_size()), std::forward_as_tuple(std::forward<Args>(args)...));
